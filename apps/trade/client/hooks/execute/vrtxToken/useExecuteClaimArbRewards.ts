...
import { accountArbRewardsClaimStateQueryKey } from 'client/hooks/query/arbRewards/useAccountArbRewardsClaimState';

const REFETCH_QUERY_KEYS: string[][] = [
  allDepositableTokenBalancesQueryKey(),
  accountArbRewardsClaimStateQueryKey(),
];

/**
 * Claims any ARB rewards
 */
export function useExecuteClaimArbRewards() {
  const mutationFn = useExecuteInValidContext(
    useCallback(async (params: EmptyObject, context: ValidExecuteContext) => {
      console.log('Claiming Arb Rewards', params);
      return context.vertexClient.rewards.claimArbRewards();
    }, []),
  );
  ...
  const mutation = useMutation({
    ...
    onError(error, variables) {
      logExecuteError('ClaimArbRewards', error, variables);
    },
  });
