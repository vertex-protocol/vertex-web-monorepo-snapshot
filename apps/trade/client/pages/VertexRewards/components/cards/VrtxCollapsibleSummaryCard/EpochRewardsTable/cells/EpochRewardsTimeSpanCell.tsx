...
interface EpochRewardsTimeSpanCellProps extends TableCellProps {
  start: number;
  end: number;
}

export function EpochRewardsTimeSpanCell({
  start,
  end,
  ...rest
}: EpochRewardsTimeSpanCellProps) {
  return (
    <TableCell {...rest}>
      <div className="flex flex-col gap-y-1.5">
        <DateTimeItem
          icon={
            <Icons.MapPinSimple className="shrink-0 -rotate-90" size={15} />
          }
          timestampMillis={start}
        />
        <DateTimeItem
          icon={<Icons.MapPinSimple className="shrink-0 rotate-90" size={15} />}
          timestampMillis={end}
        />
      </div>
    </TableCell>
