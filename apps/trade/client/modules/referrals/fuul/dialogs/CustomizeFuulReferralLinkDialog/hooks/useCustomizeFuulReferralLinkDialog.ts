...
import { useExecuteUpsertFuulAffiliateCode } from 'client/modules/referrals/fuul/hooks/execute/useExecuteUpsertFuulAffiliateCode';
...
export type CustomizeFuulReferralLinkErrorType =
...
export function useCustomizeFuulReferralLinkDialog() {
  const { dispatchNotification } = useNotificationManagerContext();
  const { hide } = useDialog();

  const executeUpsertAffiliateCode = useExecuteUpsertFuulAffiliateCode();
  ...
  const codeError: CustomizeFuulReferralLinkErrorType | undefined =
    watchFormError(form, 'code');
  ...
  const validateReferralCode = useCallback(
    async (
      code: string,
    ): Promise<CustomizeFuulReferralLinkErrorType | undefined> => {
